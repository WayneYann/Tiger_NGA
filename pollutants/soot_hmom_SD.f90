    ...
    select case(trim(chemistry))
       case ('chemtable')
          ...
          if (use_PAH) then
             ...
             srcSC_(isc_PAH) = srcSC_(isc_PAH) + dt_*rhodot*min((SC_(isc_PAH)/ypah)**2,1.0e4_WP)
          end if
       case ('finite chem')
          ! PAH equations
      ...
      case ('chemtable')
         ...
         if (use_pah) then
            ...
            do k=kmin_,kmax_
               do j=jmin_,jmax_
                  do i=imin_,imax_
                     tmp7(i,j,k) = tmp7(i,j,k) * min((SC_(i,j,k,isc_PAH) / tmp8(i,j,k))**2,1.0e4_WP)
                  end do
               end do
            end do
            !$OMP END PARALLEL DO
