...
subroutine inflow_velocity
  ...
  call parser_read('Oscillation phase',phase,0.0_WP)
  ...
  do nflow=1,ninlet
     do j=max(jmino_,inlet(nflow)%jmino),min(jmaxo_,inlet(nflow)%jmaxo)
        ...
        do k=kmino_,kmaxo_
           ...
           do i=imino,imin-1
              U(i,j,k)    = Uin(j,k) + amp * sin(twoPi * frq * time + phase / 360.0_WP * twopi) ! phase in degrees
              rhoU(i,j,k) = (Uin(j,k) + amp * sin(twoPi * frq * time + phase / 360.0_WP * twopi)) * RHOmid(imino,j,k)
              V(i,j,:)    = Vin(j,k)
           ...
           U(imin,j,k) = Uin(j,k) + amp * sin(twoPi * frq * time + phase / 360.0_WP * twopi)
           rhoU(imin,j,k) = (Uin(j,k) + amp * sin(twoPi * frq * time + phase / 360.0_WP * twopi)) * RHOmid(imino,j,k)
        end do
        !$OMP END PARALLEL DO
     end do
  end do
