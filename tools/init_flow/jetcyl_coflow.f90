module jetcyl_coflow
  ...
  integer :: pipe_points, wall_points, refine_times, fine_points_inner, fine_points_outer
  ! Radial stretching
  real(WP) :: s
...
subroutine jetcyl_coflow_grid
  ...
  call parser_read('Fine points inner',fine_points_inner,0)
  call parser_read('Fine points outer',fine_points_outer,0)
...
!!$     print*,'Zone_3 (0.97-1.03):', (x(xz_1+zone_points+2)-x(xz_1+zone_points+1))/(zone_length/real(zone_points,WP))
  ...
  if (fine_points_inner .ne. 0 .or. fine_points_outer .ne. 0) then
     dytmp = 0.5_WP*(ann_inner-pipe_diameter)/real(wall_points,WP)
     wall_points = wall_points + fine_points_inner + fine_points_outer
     pipe_diameter = pipe_diameter - 2.0_WP * dytmp * fine_points_inner
     ann_inner = ann_inner + 2.0_WP * dytmp * fine_points_outer
  end if

  ! Central jet
  s = 0.99   ! stretch rate y(j) = A + B * S**j dytmp = B  DEFAULT: 0.97
  ...
  do j=pipe_points+2,pipe_points+wall_points
     y(j) = y(j-1) + 0.5_WP*(ann_inner-pipe_diameter)/real(wall_points,WP)
     print*, 'wall and fine region', j, y(j)
  end do
  ! Annulus
  s = 1.03   ! DEFAULT: 1.03
...
! SD if double the mesh, make sure that it has two points in the inlet
  if (fine_points_inner .ne. 0 .or. fine_points_outer .ne. 0) then
     print*, 'Fine region is (',y(pipe_points+1),'to',y(pipe_points+wall_points+1) ,')'
     wall_points = wall_points - fine_points_inner - fine_points_outer
     pipe_points = pipe_points + fine_points_inner
  end if
...
subroutine jetcyl_coflow_data
  ...
  case ('finite chem')
     ! Allocate the array data
     print*, 'Should have 7+nSpecies+1 variables'
     nvar = 7 + 45 + 1 !47 ! 7 + N species + T 
     allocate(data(nx,ny,nz,nvar))
     allocate(names(nvar))
