program createUnsteadyChemtable
  use unsteady_table
  use parser
  use precision
  use string
  ...
  call unsteady_flamelet_init
  call unsteady_table_init
  ...
  do n=1,nfiles
     ...
     ! Read the file
     call unsteady_flamelet_readfile(files(n))
     ! Convolute with PDF
     call unsteady_table_convolute(n)
     ! Deallocate the data array
     call unsteady_flamelet_cleanup
  end do
  
  ! Change the variables names
  call unsteady_table_convert_names
  ! Compute the table
  call unsteady_table_setup
  ! Extent the bounds of the chemtable
  !call unsteady_table_extent
  ! Print some statistics
  call unsteady_table_stats
  ! Write the table
  call unsteady_table_write

end program createUnsteadyChemtable
