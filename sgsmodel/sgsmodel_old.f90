...
contains  
  ...
  subroutine compute_filtered_velocity
    ...
    do k=kmino_,kmaxo_
       do j=jmino_,jmaxo_
          do i=imino_,imaxo_
             FUi(i,j,k) = Ui(i,j,k)*RHO(i,j,k)
             FVi(i,j,k) = Vi(i,j,k)*RHO(i,j,k)
             FWi(i,j,k) = Wi(i,j,k)*RHO(i,j,k)
    ...
    do k=kmin_,kmax_
       do j=jmin_,jmax_
          do i=imin_,imax_
             ...
             buffer1 = S(i-1:i+1,j-1:j+1,k-1:k+1) * RHO(i-1:i+1,j-1:j+1,k-1:k+1)
             ...
             buffer1 = RHO(i-1:i+1,j-1:j+1,k-1:k+1) * Ui(i-1:i+1,j-1:j+1,k-1:k+1)
             buffer2 = RHO(i-1:i+1,j-1:j+1,k-1:k+1) * Vi(i-1:i+1,j-1:j+1,k-1:k+1)
             buffer3 = RHO(i-1:i+1,j-1:j+1,k-1:k+1) * Wi(i-1:i+1,j-1:j+1,k-1:k+1)
  ...
  subroutine sgsmodel_LM_MM_DIFF(isc)
    ...
    real(WP), dimension(3) :: Li,Mi,FGi,FrhoSGi,FrhoUiSC
    integer :: i,j,k
    ...
    !$OMP PARALLEL DO PRIVATE(buffer1,buffer2,FGi,FrhoSGi,Mi,FrhoUiSC,Li)
    do k=kmin_,kmax_
       do j=jmin_,jmax_
          do i=imin_,imax_
             ...
             ! Filter the product rho.S.Gi
             buffer1 = S(i-1:i+1,j-1:j+1,k-1:k+1) * RHO(i-1:i+1,j-1:j+1,k-1:k+1)
             buffer2 = buffer1*Gi1(i-1:i+1,j-1:j+1,k-1:k+1); call filter_local_3D(buffer2, FrhoSGi(1),i,j,'d')
             buffer2 = buffer1*Gi2(i-1:i+1,j-1:j+1,k-1:k+1); call filter_local_3D(buffer2, FrhoSGi(2),i,j,'d')
             buffer2 = buffer1*Gi3(i-1:i+1,j-1:j+1,k-1:k+1); call filter_local_3D(buffer2, FrhoSGi(3),i,j,'d')
             
             ! Compute Mi = <rho.S.Gi> - ratio^2.<rho>.<S>.<Gi>
             Mi = 2.0_WP * delta_3D(i,j)**2 * (FrhoSGi - ratio_3D(i,j)**2  * FRHO(i,j,k) * FS(i,j,k) * FGi)
             
             ! Filter the product rho.Ui.SC
             ! -> Has to be Dirichlet here for consistency with FUi,FVi,FWi
             buffer1 = RHO(i-1:i+1,j-1:j+1,k-1:k+1) * SC(i-1:i+1,j-1:j+1,k-1:k+1,isc)
             buffer2 = buffer1*Ui(i-1:i+1,j-1:j+1,k-1:k+1); call filter_local_3D(buffer2, FrhoUiSC(1),i,j,'d')
             buffer2 = buffer1*Vi(i-1:i+1,j-1:j+1,k-1:k+1); call filter_local_3D(buffer2, FrhoUiSC(2),i,j,'d')
             buffer2 = buffer1*Wi(i-1:i+1,j-1:j+1,k-1:k+1); call filter_local_3D(buffer2, FrhoUiSC(3),i,j,'d')
             
             ! Compute Li = <rho.Ui.SC> - <rho>.<Ui>.<SC>
             Li(1) = FrhoUiSC(1) - FrhoSC(i,j,k) * FUi(i,j,k)
             Li(2) = FrhoUiSC(2) - FrhoSC(i,j,k) * FVi(i,j,k)
             Li(3) = FrhoUiSC(3) - FrhoSC(i,j,k) * FWi(i,j,k)
